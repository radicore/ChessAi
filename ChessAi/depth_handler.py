...
def set_depth(BOARD, engineType=2):
    ...
    if engineType == 2:
        if mc in [3, 4]:
            return 6
        elif mc in [i for i in range(5, 16)]:
            return 5
        else:
            return 3
    else:
...
def optimal_move(max_depth, BOARD, end_game=False, engineType=2, debug=False, processes=4):
    ...
    if BOARD.fen() in ["8/8/8/8/3K4/2R5/1k6/8 w - - 0 1", "8/1k6/2R5/3K4/8/8/8/8 w - - 0 1", "8/6k1/5R2/4K3/8/8/8/8 w - - 0 1", "8/8/8/8/4K3/5R2/6k1/8 w - - 0 1"]:
        step = 10
        print("DEPTH SET TO 10 (KNOWN CHECKMATE POSITION) Calculating moves...")
                ...
                if BOARD.turn:
                    if EVAL > best_eval:
                        ...
                        if EVAL == INF:
                            ...
                            if debug:
                                print(f"M{true_depth} MOVE(S) - {move}")
                            return move, EVAL
                else:
                    if EVAL < best_eval:
                        best_eval = EVAL
